...
def sortClusterVariables(_function, _primal_graph_edges, _cluster)
  ...
  sorted_vertex = vertex_weight.sort_by { |key, value| [-value,key.to_i] }
  ...
  adjacent_central_vertex.sort_by { |key, value|[-value,key.to_i] }.each do |pair|
    ret << pair[0]
    vertex_weight.delete(pair[0])
  end

  extra_vertex = vertex_weight.sort_by { |key, value| [-value,key.to_i] }
  ...
  while vertex_to_process.size() > 0
    ...
    vertex_list.sort_by { |key, value| [-value, key.to_i] }.each do |vertex|
      ret << vertex[0]
    end
...
def getNarodystka(_function,_mcl_i=2.5)
  ...
  ret = Array.new()
  projection.sort_by { |key, value| [-value,key.to_i] }.each do |pair|
