...
class FUNCTION
  ...
  def get_optimal_groups!(_variables,symmetric_groups,_file_name)
    ...
    minisat_output = `#{cmd}`
    if self.name.include?("Dell")
      minisat_output =~ /s SATISFIABLE\s+v\s+(.+?)\s+c _______________________________________________________________________________/m
    else
...
=end
  ...
  def loadFromSPLOTFile(_input_file)
    ...
    if File.exist?(_input_file)
      ...
      if @minisat
        get_optimal_groups!(ret.variables,ret.symmetric_groups,_input_file)
      end
      #ROB : 01/11/2016 Get optimals groups
